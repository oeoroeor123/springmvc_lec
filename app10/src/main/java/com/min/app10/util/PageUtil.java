...
public class PageUtil {
  ...
  private int total; // 전체 목록의 갯수 (SELECT 문 실행하여 확인 가능)
  private int totalPage; // 전체 페이지 갯수 (total과 display에 의해서 계산되는 항목) ex_total = 1000, display = 20 >> tatalPage = 50;
  ...
  public void setPaging(int page, int display, int total) {
    ...
    endPage = Math.min(beginPage + pagePerBlock -1, totalPage); // Math.min : 두개 중 작은 값을 endPage로 쓴다.
